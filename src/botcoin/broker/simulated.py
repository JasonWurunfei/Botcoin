...
from botcoin.data.dataclasses import Order, OrderStatusEvent, PlaceOrderEvent, OrderStatus
...
class SimpleBroker:
    ...
    async def on_order(self, order_event: PlaceOrderEvent) -> None:
        ...
        if reply_to is not None:
            ...
            self.logger.debug(f"Order {order.order_id} processed with status: {res['status'].value}")
    ...
    async def simulate_trade(self, order: Order, reply_to: Queue) -> None:
        ...
        await reply_to.put(order_status_event)
        self.logger.debug(f"Order {order.order_id} traded with status: {order_status_event.status.value}")
